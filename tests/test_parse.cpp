...
// Mirrors the uriMHDA corpus of the Go reference implementation.
...
TEST_CASE("sentinel error codes") {
    ...
    std::vector<row> cases = {
        ...
        {"urn:mhda:nt:notanetwork:ci:1:ct:60",                  error_code::invalid_network_type},
        {"urn:mhda:nt:evm:ci:1:ct:notanumber",                  error_code::invalid_coin_type},
        {"urn:mhda:nt:evm:ct:60",                               error_code::missing_chain_id},
        {"urn:mhda:nt:evm:ci:1:ct:60:dt:bipxx:dp:m/44'/60'/0'/0/0",
                                                                error_code::invalid_derivation_type},
        {"urn:mhda:nt:evm:ci:1:ct:60:dt:bip44:dp:not_a_path",   error_code::invalid_derivation_path},
        {"urn:mhda:nt:evm:ci:1:ct:60:aa:rsa",                   error_code::invalid_algorithm},
        {"urn:mhda:nt:evm:ci:1:ct:60:af:notaformat",            error_code::invalid_format},
    };
    for (const auto& c : cases) {
        EXPECT_THROW_CODE(parse_urn(c.in), c.want);
    }

    // The strict chain-key parser has two dedicated sentinels of its own.
    EXPECT_THROW_CODE(chain::from_key("nt:evm:ct:60:ci:1"),
                      error_code::coin_type_in_chain_key);
    EXPECT_THROW_CODE(chain::from_key("nt:evm:ci:1:aa:secp256k1"),
                      error_code::invalid_chain_key);
}

TEST_CASE("RFC 8141 case-insensitive prefix") {
    const std::string canonical = "urn:mhda:nt:evm:ci:1:ct:60";
...
TEST_CASE("hardened marker normalisation") {
    auto a = parse_urn("urn:mhda:nt:evm:ci:1:ct:60:dt:bip44:dp:m/44H/60H/0H/0/0");
    EXPECT_EQ(a.str(), std::string{"urn:mhda:nt:evm:ci:1:ct:60:dt:bip44:dp:m/44'/60'/0'/0/0"});
    auto b = parse_urn("urn:mhda:nt:evm:ci:1:ct:60:dt:bip44:dp:m/44h/60h/0h/0/0");
    EXPECT_EQ(b.str(), std::string{"urn:mhda:nt:evm:ci:1:ct:60:dt:bip44:dp:m/44'/60'/0'/0/0"});
}
...
TEST_CASE("derivation type accessor") {
    auto a = parse_urn("urn:mhda:nt:evm:ci:1:ct:60:dt:bip44:dp:m/44'/60'/0'/0/0");
    EXPECT_EQ(a.get_derivation_type(), derivation_type::bip44);
    auto root = parse_urn("urn:mhda:nt:evm:ci:1:ct:60");
    EXPECT_EQ(root.get_derivation_type(), derivation_type::root);
}

TEST_CASE("regression: whitespace before fragment") {
    const std::string in = "urn:mhdA:nt:BitCoin:ct:0:ci:0 #";
...
TEST_CASE("set prefix/suffix reset") {
    address a{
        chain{network_type::ethereum_vm, "1"},
        std::nullopt,
        "", "", "0xPREFIX", "SUFFIX",
    };
